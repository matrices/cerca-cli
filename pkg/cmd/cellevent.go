...
var cellsEventsList = cli.Command{
	Name:    "list",
	...
	Action:          handleCellsEventsList,
	HideHelpCommand: true,
}

var cellsEventsSubscribe = cli.Command{
	Name:    "subscribe",
	...
	Action:          handleCellsEventsSubscribe,
	HideHelpCommand: true,
}

func handleCellsEventsList(ctx context.Context, cmd *cli.Command) error {
	...
	params := cercago.CellEventListParams{}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Title:          "cells:events list",
		Transform:      transform,
	})
}

func handleCellsEventsSubscribe(ctx context.Context, cmd *cli.Command) error {
	client := cercago.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("cell-id") && len(unusedArgs) > 0 {
		cmd.Set("cell-id", unusedArgs[0])
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Cells.Events.Subscribe(ctx, cmd.Value("cell-id").(string), options...)
	if err != nil {
		return err
	}
