...
var environmentsEventsList = cli.Command{
	Name:    "list",
	...
	Action:          handleEnvironmentsEventsList,
	HideHelpCommand: true,
}

var environmentsEventsSubscribe = cli.Command{
	Name:    "subscribe",
	...
	Action:          handleEnvironmentsEventsSubscribe,
	HideHelpCommand: true,
}

func handleEnvironmentsEventsList(ctx context.Context, cmd *cli.Command) error {
	...
	params := cercago.EnvironmentEventListParams{}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Title:          "environments:events list",
		Transform:      transform,
	})
}

func handleEnvironmentsEventsSubscribe(ctx context.Context, cmd *cli.Command) error {
	client := cercago.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("env-id") && len(unusedArgs) > 0 {
		cmd.Set("env-id", unusedArgs[0])
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Environments.Events.Subscribe(ctx, cmd.Value("env-id").(string), options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "environments:events subscribe",
		Transform:      transform,
	})
}
