...
var cellsThreadsApprovalGrantsList = cli.Command{
	Name:    "list",
	...
	Action:          handleCellsThreadsApprovalGrantsList,
	HideHelpCommand: true,
}

var cellsThreadsApprovalGrantsDelete = cli.Command{
	Name:    "delete",
	...
	Action:          handleCellsThreadsApprovalGrantsDelete,
	HideHelpCommand: true,
}

func handleCellsThreadsApprovalGrantsList(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Title:          "cells:threads:approval-grants list",
		Transform:      transform,
	})
}

func handleCellsThreadsApprovalGrantsDelete(ctx context.Context, cmd *cli.Command) error {
	client := cercago.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("cell-id") && len(unusedArgs) > 0 {
		cmd.Set("cell-id", unusedArgs[0])
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "cells:threads:approval-grants delete",
		Transform:      transform,
	})
}
